...
- **追加修复（线上看不到新功能的根因 = 浏览器缓存）**：`/static/*.jsx|css` 原先只带 ETag/Last-Modified 无 `Cache-Control`，浏览器走启发式缓存，部署后仍加载旧文件。修复：① `api.py` 加中间件，对 `/` 和 `/static/*` 发 `Cache-Control: no-cache`（始终回源校验，命中则 304，很轻量）；② `index.html` 9 个静态资源加 `?v=20260608` 一次性破除已缓存旧文件。此后每次部署普通刷新即生效，无需硬刷新。已在真实浏览器打开线上 `:3000` 任务 #18 验证：20 条标题全部渲染为链接、href 指向真实沃尔玛详情页、新标签打开、带外链图标。
